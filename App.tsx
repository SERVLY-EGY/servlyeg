      ...
      <main className="max-w-7xl mx-auto px-6 py-8">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-8 text-gray-900 dark:text-gray-100">
          ...
          >
            ...
            <div className="flex flex-wrap gap-2">
              <span className="px-3 py-1 bg-purple-100 text-purple-800 text-xs rounded-full">{t.loyaltyPoints}</span>
      ...
      <nav className="fixed bottom-6 left-1/2 transform -translate-x-1/2 backdrop-blur-xl bg-white/80 rounded-2xl border border-white/20 shadow-lg p-2 z-50">
        <div className="flex items-center space-x-2">
          <button
            ...
            className={`p-3 rounded-xl transition-all ${currentPage === 'home' ? 'bg-purple-100 text-purple-600' : 'text-gray-600 hover:bg-gray-100'}`}
          >
            <Home className="w-5 h-5" />
          </button>
          <button
            onClick={() => setCurrentPage('hr')}
            className={`p-3 rounded-xl transition-all ${currentPage === 'hr' ? 'bg-purple-100 text-purple-600' : 'text-gray-600 hover:bg-gray-100'}`}
          >
            <Users className="w-5 h-5" />
          </button>
          <button
            onClick={() => setCurrentPage('loyalty')}
            className={`p-3 rounded-xl transition-all ${currentPage === 'loyalty' ? 'bg-purple-100 text-purple-600' : 'text-gray-600 hover:bg-gray-100'}`}
          >
            <Gift className="w-5 h-5" />
          </button>
          <button
            onClick={() => setCurrentPage('website')}
            className={`p-3 rounded-xl transition-all ${currentPage === 'website' ? 'bg-purple-100 text-purple-600' : 'text-gray-600 hover:bg-gray-100'}`}
          >
            <Globe className="w-5 h-5" />
          </button>
          <button
            onClick={() => setCurrentPage('subscriptions')}
            className={`p-3 rounded-xl transition-all ${currentPage === 'subscriptions' ? 'bg-purple-100 text-purple-600' : 'text-gray-600 hover:bg-gray-100'}`}
          >
